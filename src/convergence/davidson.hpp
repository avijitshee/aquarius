...
class Davidson : public task::Destructible
{
    ...
    public:
        ...
        template <typename c_container, typename hc_container>
        std::enable_if_t<is_same<typename std::decay_t<c_container>::value_type, T>::value, vector<dtype>>
        ...
        template <typename c_container, typename hc_container>
        std::enable_if_t<is_same<typename std::decay_t<c_container>::value_type::value_type, T>::value, vector<dtype>>
        extrapolate(c_container&& c, hc_container&& hc, const op::Denominator<dtype>& D)
        {
            using slice::all;
        ...
        std::enable_if_t<is_same<typename std::decay_t<guess_container>::value_type,T>::value>
        ...
        template <typename guess_container>
        std::enable_if_t<is_same<typename std::decay_t<guess_container>::value_type::value_type,T>::value>
        reset(const guess_container& gs, int nc = 1, InnerProd innerProd = InnerProd(), Weight weight = Weight())
        {
            this->nc = nc;
            this->innerProd = innerProd;
